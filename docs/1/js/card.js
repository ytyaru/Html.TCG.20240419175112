...
class Skill {
    ...
    constructor(name, type=Skill.Type.Attack, target=Skill.Target.Front, point=1, rate=100, frequency=1) {
        this.name = name           // 名
        this.type = type           // 種類
        this.target = target       // 対象（どの席にいる、誰に、何人（深さdepth、範囲range（距離1〜5、方向(上下)）））
        this.point = point         // 点
        this.rate = rate           // 成功率
        this.frequency = frequency // 実行回数
        // 発動時期:Activation time 開/始/備/動/終/閉
    ...
    getTargets(turns, side, attacker, defender, f) { // 対象決定
        if (Skill.Type.Attack===this.type) {
            if (Skill.Target.Front===this.target) {
                ...
                return [((0 < field.servants.length) ? field.servants[0] : defender.master)]
            }
        }
        return []
    }
...
const c2 = new Card(
    ...
    new Flavour('ただの鼠。'), 
    new Status(1, 1),
    [new Skill('齧る',Skill.Type.Attack, Skill.Target.Front, 1, 50, 1)])
